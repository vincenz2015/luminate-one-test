    ...
    <div>
        ...
        <div class="form-container">
            <form class="form" @submit="update">
                <h1>Change {{ resource.name }}</h1>
                <div class="input-group">
                    <div class="w-80 d-inline-block">
                        <h4>RESOURCE NAME</h4>
                        <input v-model="resource.name" type="text" class="form-control">
                    </div>
                    <div class="w-20 d-inline-block">
                        <h4>ICON</h4>
                        <emoji-selector v-model="resource.icon"></emoji-selector>
                    </div>
                ...
                <div class="input-group">
                    <div class="w-100 d-inline-block">
                        ...
                        <input type="checkbox" v-model="resource.archived" class="form-control">
                    </div>
                </div>
                <br>
                <br>
        ...
        beforeCreate() {
            this.$hub.emit('load resources').then(() =>
                this.resource = Object.assign({},
                    this.resourcesById[this.resource.id]))
        },
        computed: {
            resourcesById() {
            ...
            backHref() {
                return `/resource/${this.resource.id}`
            }
        },
        methods: {
            update(e) {
                e.preventDefault()
                this.$hub.emit('update resource', this.resource)
                    .then((response) => {
